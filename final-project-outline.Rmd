...
```{r}
# Calculate average CO2 emissions per country within each income group
...
ggplot(co2_income_long) +
  ...
  geom_line(data = sum_co2_all_year, aes(x = CountryName, y = `CO2(metric tons per capita)`, color = IncomeGroup, group = IncomeGroup), show.legend = FALSE) +
  labs(
    x = "Country Name",
    y = "CO2 (metric tons per capita)",
    title = "Countries by Income Group vs CO2 Emissions for All A years"
  ) +
  theme_minimal() +
  theme(
